...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 21);
...
unsigned int nStakeMaxAge = 60 * 60 * 24 * 90;	// stake age of full weight:  3 months
unsigned int nStakeTargetSpacing = 2 * 60;			// 2 minutes block spacing

int64 nChainStartTime = 1404837825;
...
const string strMessageMagic = "Noahcoin Signed Message:\n";
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Noahcoin
    bool fStrictPayToScriptHash = true; // Always active in Noahcoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Noahcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
		...
		nModifierInterval = 60; // test modifier interval is 1 minute
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Rain is coming";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1404837825;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 27205039;
		if(fTestNet)
        {
            block.nNonce   = 12006;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xd643af9215867e697a86294840088458bd52f83d01bcce9ddf8d347626feb5f6"));
...
unsigned char pchMessageStart[4] = { 0x21, 0x35, 0x22, 0xdb };
